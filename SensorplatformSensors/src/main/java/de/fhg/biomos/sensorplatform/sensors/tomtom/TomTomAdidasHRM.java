package de.fhg.biomos.sensorplatform.sensors.tomtom;
...
/**
 * @see {@link de.fhg.biomos.sensorplatform.sensors.tomtom.TomTomAndAdidasHRMlib}
 *
 * @author Daniel Pyka
 *
 */
public class TomTomAdidasHRM extends HeartRateSensor {

  private static final Logger LOG = LoggerFactory.getLogger(TomTomAdidasHRM.class);
  ...
  public TomTomAdidasHRM(Properties properties, SensorName name, String bdAddress, AddressType addressType, JSONObject sensorConfiguration) {
  ...
  private void enableHeartRateNotification(String charWriteCmd, String enableNotification) {
    try {
      this.bw.write(charWriteCmd + " " + TomTomAndAdidasHRMlib.HANDLE_HEART_RATE_NOTIFICATION + " " + enableNotification);
      this.bw.newLine();
      this.bw.flush();
  ...
  private void disableHeartRateNotification(String charWriteCmd, String disableNotification) {
    try {
      this.bw.write(charWriteCmd + " " + TomTomAndAdidasHRMlib.HANDLE_HEART_RATE_NOTIFICATION + " " + disableNotification);
      this.bw.newLine();
      this.bw.flush();
